...
public class HtmlMatcher {
    ...
    public void match() throws IOException{	
		...
		//System.out.println(content); // This is for testing.
		...
		while((indexOfOpen = content.indexOf("<", indexOfOpen)) != -1)
		{
			...
			int indexOfClose = 0;
			//1.**here
			indexOfClose = content.indexOf(">", indexOfOpen);
			
			String fullTag = null;
			//2.**here
			fullTag = content.substring(indexOfOpen, indexOfClose+1);
			// System.out.println(fullTag); // This is for testing.
			...
			if((indexOfSpace = fullTag.indexOf(" ")) == -1)
			{
				...
				//For example, if fullTag is  "</li>" then the tagName will be "/li"
				//(Note that we preserve the slash'/' so we can tell that this is a close tag in the future)
				
				// I currently can't really understand what methods to implement yet.
				//3.**here
				//System.out.print(fullTag.length()+"   ");
				tagName = fullTag.substring(1, fullTag.length()-1);
				//System.out.println(tagName); // This is for testing.
			...
			else
			{
				...
				// I understand the methodology behind all these
				
			    //4.**here
				//System.out.print(fullTag);
			...
			if(tagName.equals("meta") || tagName.equals("!doctype") || tagName.equals("!DOCTYPE") || tagName.equals("link"))
			{
