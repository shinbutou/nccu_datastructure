...
public class HtmlMatcher {
    ...
    public void match() throws IOException{	
		...
		while((indexOfOpen = content.indexOf("<",indexOfOpen))!=-1){
			...
			if(indexOfSpace == -1){
				int indexOfClose_1 = fullTag.indexOf(">");
				...
				//3.**here
				tagName = fullTag.substring(1,indexOfClose_1);
			}else{
			    ...
			    //4.**here
				//content.substring(indexOfOpen+1)
				tagName = fullTag.substring(1, indexOfSpace);
			}
			if(tagName.equals("meta") || tagName.equalsIgnoreCase("!doctype")||tagName.contains("--")||tagName.equalsIgnoreCase("link")) {
